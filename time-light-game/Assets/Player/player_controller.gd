...
var arms_bob_phase: float = 0.0
var arms_rig_base_y: float = 0.0
# the vhs post effect handles the screen tint and flashes now
@onready var vhs_material: ShaderMaterial = $PlayerCharacterBody3D/PlayerCamera/CanvasLayer/ColorRect.material
@onready var hurt_audio: AudioStreamPlayer3D = $PlayerCharacterBody3D/HurtSFX
...
@onready var aim_ray: RayCast3D = $PlayerCharacterBody3D/PlayerCamera/AimRay
...
var sprint_grace_time_left: float = 0.0
# how long youve been coasting overspeed with no input, drives the exponential decel
var overspeed_coast_time: float = 0.0
var base_floor_snap: float = 0.1
var camera_base_height: float = 0.0
var camera_current_height: float = 0.0
var camera_step_offset: float = 0.0
# downward camera dip from a hard landing, eases back to 0
var camera_land_offset: float = 0.0
...
func _ready() -> void:
	...
	base_floor_snap = character_body.floor_snap_length
	camera_base_height = player_camera.position.y
	camera_current_height = camera_base_height

	overlay_material = overlay_mesh.mesh.material as ShaderMaterial
	# start from no tint, alpha 0 is the off state now that alpha is strength
	overlay_material.set_shader_parameter("tint", Color(1, 1, 1, 0))
	vhs_material.set_shader_parameter("tint", Color(1, 1, 1, 0))
	vhs_material.set_shader_parameter("flash", Color(1, 1, 1, 0))
	overlay_was_visible = overlay_mesh.visible
	...
	arms_animation_player.animation_finished.connect(on_arms_animation_finished)
	# remember where the arms sit so the running bob can offset from it
	arms_rig_base_y = arms_rig.position.y
...
func _process(delta: float) -> void:
	...
	update_guard_animation()
	update_arm_bob(delta)
	...
	arms_idle_timer += delta
	if arms_idle_timer >= arms_idle_delay:
		arms_idle_timer = 0.0
...
func update_arm_bob(delta: float) -> void:
	...
	if bobbing:
		arms_bob_phase += delta * arms_bob_speed
		arms_rig.position.y = arms_rig_base_y + sin(arms_bob_phase) * arms_bob_amount
	else:
		arms_rig.position.y = move_toward(arms_rig.position.y, arms_rig_base_y, arms_bob_amount * 8.0 * delta)


# Crouch, slide and slam all raise the guard, dropping slide or crouch reverses the raise
func update_guard_animation() -> void:
	var guard_now := crouch_slide.is_crouching or crouch_slide.is_sliding or crouch_slide.is_slamming

	if guard_now and not guarding:
		guarding = true
		if arms_animation_player.has_animation(guard_draw_animation):
			arms_animation_player.play(guard_draw_animation)
		return

	if not guard_now and guarding:
		guarding = false
		# reverse the draw
		if arms_animation_player.has_animation(guard_draw_animation):
...
func on_arms_animation_finished(anim_name: StringName) -> void:
	if anim_name == guard_draw_animation and guarding:
...
func _physics_process(delta: float) -> void:
	...
	if pause_layer != null or dead or Global.is_time_stopped():
		return
	
	var input_vector := Input.get_vector(&"ui_left", &"ui_right", &"ui_up", &"ui_down")
	
	# Sets sprint state and updates sprint grace
	update_sprint_state(delta, input_vector)
	...
	crouch_slide.update_input(delta, is_on_floor, current_move_direction, is_sprinting)
	...
	update_horizontal_movement(delta, current_move_direction, is_on_floor)
	...
	character_body.floor_snap_length = slide_floor_snap_length if crouch_slide.is_sliding else base_floor_snap

	var saved_floor_snap := character_body.floor_snap_length
	if step_height > 0.0:
		camera_step_offset -= step_height
	...
	if not is_on_floor and character_body.is_on_floor() and impact_speed > land_bob_min_speed: # <- this is never true 
		...
		camera_land_offset = -dip

	var horizontal_speed := Vector2(character_body.velocity.x, character_body.velocity.z).length()
	footsteps.post_move_update(delta, horizontal_speed > 0.1, is_sprinting)
...
func update_sprint_state(delta: float, input_vector: Vector2) -> void:
	...
	if sprint_pressed and not crouch_slide.is_crouching:
		sprint_grace_time_left = sprint_release_grace_time
	else:
		sprint_grace_time_left = maxf(sprint_grace_time_left - delta, 0.0)

	is_sprinting = (Input.is_physical_key_pressed(KEY_SHIFT) or sprint_grace_time_left > 0.0) and !crouch_slide.is_crouching



func clamp_horizontal_speed() -> void:
...
func update_horizontal_movement(delta: float, current_move_direction: Vector3, was_on_floor: bool) -> void:
	...
	if current_speed > sprint_speed + 0.001:
		...
		var bleed := 0.0
		if was_on_floor:
			bleed = sprint_momentum_bleed if is_sprinting else walk_momentum_bleed
...
func _apply_preserved_speed(
	...
	if move_direction.is_zero_approx():
		if grounded:
			...
			overspeed_coast_time += delta
			var decel := overspeed_deceleration * exp(overspeed_decel_growth * overspeed_coast_time)
...
func _try_interact() -> void:
	var from := aim_ray.global_position
	var to := from - aim_ray.global_transform.basis.z * interact_range
...
func heal_to_full() -> void:
	...
	_update_health_label()
	_flash_heal()
	hurt_audio.stream = HEAL_SFX
	hurt_audio.pitch_scale = 1.0
	hurt_audio.play()
...
func _flash_heal() -> void:
	...
	vhs_material.set_shader_parameter("flash", Color(0.2, 1.0, 0.3, 0.45))
	hit_flash_tween = create_tween()
	hit_flash_tween.tween_property(vhs_material, "shader_parameter/flash", Color(0.2, 1.0, 0.3, 0.0), 0.35)


func take_damage(amount) -> void:
	# untouchable mid dash or mid i frames, or already down
	if invuln_time > 0.0 or health <= 0:
		return
	health = maxi(health - int(amount), 0)
	# i frames so a swarm cant chain hits in one instant
	invuln_time = maxf(invuln_time, hit_invuln_time)
	_update_health_label()
	_flash_hit()
	_play_hurt_sfx()
	if health <= 0:
...
func _play_hurt_sfx() -> void:
	if hurt_sfx == null:
		return
	hurt_audio.stream = hurt_sfx
	hurt_audio.pitch_scale = randf_range(hurt_pitch_min, hurt_pitch_max)
	hurt_audio.play()
...
func die() -> void:
	...
	# blood filter
	vhs_material.set_shader_parameter("tint", Color(1, 1, 1, 0))
	vhs_material.set_shader_parameter("flash", Color(0.75, 0.0, 0.0, 0.6))
...
func _show_death_screen() -> void:
	...
	pause_layer.process_mode = Node.PROCESS_MODE_ALWAYS
	add_child(pause_layer)
	pause_layer.add_child(DEATH_SCREEN.instantiate())
...
func _flash_hit() -> void:
	if hit_flash_tween:
		hit_flash_tween.kill()
	vhs_material.set_shader_parameter("flash", Color(0.8, 0.0, 0.0, 0.45))
	hit_flash_tween = create_tween()
	hit_flash_tween.tween_property(vhs_material, "shader_parameter/flash", Color(0.8, 0.0, 0.0, 0.0), 0.35)
...
func update_camera_height(delta: float) -> void:
	camera_step_offset = move_toward(camera_step_offset, 0.0, step_camera_smoothing * delta)
	# impact bob for landing
	camera_land_offset = lerp(camera_land_offset, 0.0, clampf(land_bob_recovery * delta, 0.0, 1.0))
	var target_height := crouch_camera_height if crouch_slide.is_crouching else camera_base_height
	camera_current_height = move_toward(camera_current_height, target_height, crouch_camera_speed * delta)
	var camera_position := player_camera.position
	camera_position.y = camera_current_height + camera_step_offset + camera_land_offset
...
func _on_game_speed_state_changed(new_state) -> void:
	...
	if new_state == Global.TimeState.STOPPED:
		...
		overlay_tween = create_tween().set_parallel()
		overlay_tween.tween_property(overlay_material, "shader_parameter/pixel_size", 2, 0.15)
		overlay_tween.tween_property(vhs_material, "shader_parameter/tint", frozen_tint, 0.15)
	else:
		# ease the pixelization back out while the vhs clears its tint and pops a
		# quick green flash on resume
		overlay_tween = create_tween().set_parallel()
		overlay_tween.tween_property(overlay_material, "shader_parameter/pixel_size", normal_pixel_size, 0.4)
		overlay_tween.tween_property(vhs_material, "shader_parameter/tint", Color(1, 1, 1, 0), 0.3)

		var flash_clear := Color(resume_flash_tint.r, resume_flash_tint.g, resume_flash_tint.b, 0.0)
		flash_tween = create_tween()
		flash_tween.tween_property(vhs_material, "shader_parameter/flash", resume_flash_tint, 0.08)
		flash_tween.tween_property(vhs_material, "shader_parameter/flash", flash_clear, 0.35)
